...
namespace ImageSharp
{
        ...
        where TPixel : struct, IPixel<TPixel>
    {
        ...
        public static Image<Rgba32> FromInts(int[] data, int width, int height)
        {
            var target = new Image<Rgba32>(width, height);
            using (PixelAccessor<Rgba32> pixels = target.Lock())
            {
                Parallel.For(
                    0,
                    target.Height,
                    target.Configuration.ParallelOptions,
                    y =>
                    {
                        for (int x = 0; x < target.Width; x++)
                        {
                            var color = default(Rgba32);
                            byte depack(int shift) => (byte)((data[x + (y * width)] & (0xFF << (shift * 8))) >> (shift * 8));
                            color.PackFromRgba32(new Rgba32(depack(3), depack(2), depack(1), depack(0)));
                            pixels[x, y] = color;
                        }
                    });
            }
